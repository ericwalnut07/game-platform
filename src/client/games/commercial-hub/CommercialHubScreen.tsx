  ...
  return <div className="hub" data-game-id="commercial-hub" data-phase={view.phase}>
    ...
    <div className="hub-layout">
      <div className={`hub-turn-column ${tab === "turn" ? "hub-visible" : ""}`}>
        <section className="hub-card hub-opportunities"><h2>このラウンドの商機</h2><ol>{view.opportunities.map((opportunity, index) => <li key={index} className={index === view.trickIndex ? "current" : index < view.trickIndex ? "past" : ""}><span>{index + 1}</span>{opportunityName(opportunity)}{index === view.trickIndex && <small>現在</small>}</li>)}</ol><p>{REWARDS[currentOpportunity]}</p></section>
        ...
        <div className="hub-card hub-credit-summary"><strong>保持中の権利</strong><p>敷設 {credits.route} · 輸送 {credits.transport}<br/>特別販売 {credits.businessSale} · 生産+1 {credits.productionBoost} · 建設資金-1 {credits.constructionCash}</p><small>未使用分は繰越。建物による軽減は毎ラウンド更新。</small></div>
      </div>
