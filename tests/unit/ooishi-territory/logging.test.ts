...
describe("ooishi-territory D1 logs", () => {
  it("stores versioned settings and finished public results without modifying Pon tables", async () => {
    ...
    await persistStateTransitionForGame(db, "ooishi-territory", initial, state, 5000, "ABC123");
    expect(sqlite.prepare("SELECT game_id, player_count, game_count, app_version, finished_at, ended_reason FROM playtest_matches WHERE match_id='territory-match'").get())
      .toMatchObject({ game_id: "ooishi-territory", player_count: 2, game_count: 1, app_version: APP_VERSION, finished_at: 5000, ended_reason: "COMPLETED" });
    ...
    expect(events.map((entry) => entry.event_type)).toEqual(["TERRITORY_CONFIG", "TERRITORY_RESULT"]);
    expect(JSON.parse(events[0]!.payload_json as string).config).toEqual(config);
    expect(JSON.parse(events[1]!.payload_json as string).moves).toBe(4);
